...
./configure \
  ...
  --with-libtiff \
  --prefix="$WORK" \
  LIBS="$LEPTONICA_LIBS -llzma" \
  LDFLAGS="-L$WORK/lib" \
  CPPFLAGS="-I$WORK/include -DFUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION" \
  CFLAGS="$CFLAGS" \
  CXXFLAGS="$CXXFLAGS"
make -j$(nproc) CFLAGS="$CFLAGS $SAVE_TEMPS" CXXFLAGS="$CXXFLAGS $SAVE_TEMPS"
make install
...
cp $SRC/leptonica/prog/fuzzing/general_corpus.zip $OUT/general_corpus.zip
